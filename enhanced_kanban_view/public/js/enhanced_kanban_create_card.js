...
function applyKanbanEnhancements() {
	frappe.provide("frappe.views");
	frappe.provide("frappe.views.KanbanView");
	...
	(function () {
			...
			actions: {
				...
				update_order: function (context) {
					...
					context.state.wrapper
						.find(".kanban-column[data-column-value]")
						.each(function () {
		...
		frappe.views.KanbanBoard = function (opts) {
			...
			function bind_add_column() {
				...
				const is_custom_field = frappe.meta.get_docfield(
					doctype,
					fieldname
				)?.is_custom_field;
		...
		frappe.views.KanbanBoardColumn = function (column, wrapper, board_perms) {
			...
			function setup_sortable() {
				...
				Sortable.create(self.$kanban_cards.get(0), {
					...
					onEnd: async function (e) {
						const from_colname = $(e.from)
							.parents(".kanban-column")
							.attr("data-column-value");
						const to_colname = $(e.to)
							.parents(".kanban-column")
							.attr("data-column-value");
						const card = get_card(decodeURIComponent($(e.item).attr("data-name")));

						// Only show dialog if moving to different column
						if (from_colname !== to_colname) {
							const additional_required_fields = await frappe.call({
								method: "enhanced_kanban_view.api.kanban_board_rule.get_additional_fields",
								args: {
									kanban_board_name: store.state.board.name,
									column_name: to_colname,
								},
							});
							frappe.updated_card = null;
							if (additional_required_fields.message.length > 0) {
								await frappe.enhanced_kanban_quick_edit(
									store.state.doctype,
									card.name,
									additional_required_fields.message,
									() => {
										frappe.updated_card = true;
									},
									{ status: to_colname }
								);
								// Wait until the quick entry dialog is closed (frappe.enhanced_kanban_quick_entry is set to null)
								let times = 0;
								while (frappe.enhanced_kanban_quick_entry !== null) {
									times++;
									await new Promise((resolve) => setTimeout(resolve, 100));
								}

								// If card was not updated, revert to original position
								if (!frappe.updated_card) {
									// Revert the card to its original position
									const $originalColumn = wrapper.find(
										`[data-column-value="${from_colname}"] .kanban-cards`
									);
									const $card = $(e.item);
			...
			function refresh_list() {
				const $refresh_button = $('[data-original-title="Reload List"]');
			...
			function bind_add_card() {
				...
				$btn_add.on("click", async function () {
					...
					frappe.ui.form.make_enhanced_kanban_quick_entry(
						store.state.doctype,
						refresh_list,
						null,
						null,
...
function patchKanbanView() {
	if (frappe.views.KanbanView && frappe.views.KanbanView.prototype.render) {
		const original_render = frappe.views.KanbanView.prototype.render;
		let overrides_applied = false;

		frappe.views.KanbanView.prototype.render = function (...args) {
			if (!overrides_applied) {
				applyKanbanEnhancements();
				overrides_applied = true;
			}
			return original_render.apply(this, args);
		};
	} else {
		setTimeout(patchKanbanView, 50);
	}
}
